...
export default function FloatingChat() {
  ...
  return (
    <div ref={containerRef} className={`fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3 ${isLevel1User ? "user-level-1-floating-chat" : ""} ${isLevel2User ? "user-level-2-floating-chat" : ""} ${isLevel3User ? "user-level-3-floating-chat" : ""}`}>
      {/* Panel principal */}
      {open && (
        <div className={`${isLevel1User ? "user-level-1-floating-chat-panel" : ""} ${isLevel2User ? "user-level-2-floating-chat-panel" : ""} ${isLevel3User ? "user-level-3-floating-chat-panel" : ""} w-80 rounded-2xl border border-white/10 shadow-2xl overflow-hidden flex flex-col`}
          style={{ background: 'rgba(15,10,30,0.97)', backdropFilter: 'blur(16px)' }}>

          {/* Header */}
          <div className={`${isLevel1User ? "user-level-1-floating-chat-header" : ""} ${isLevel2User ? "user-level-2-floating-chat-header" : ""} ${isLevel3User ? "user-level-3-floating-chat-header" : ""} flex items-center justify-between px-4 py-3 border-b border-white/10`}
          ...
          {activeFriend && (
            <>
                ...
                {messages.map(msg => {
                  ...
                  return (
                    <div key={msg.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                      <div className={`${isLevel1User ? 'user-level-1-floating-chat-message' : ''} ${isLevel2User ? 'user-level-2-floating-chat-message' : ''} ${isLevel3User ? 'user-level-3-floating-chat-message' : ''} max-w-[75%] px-3 py-2 rounded-2xl text-sm break-words ${
                        mine
                          ? `${isLevel1User ? 'user-level-1-floating-chat-message-mine' : ''} ${isLevel2User ? 'user-level-2-floating-chat-message-mine' : ''} ${isLevel3User ? 'user-level-3-floating-chat-message-mine' : ''} text-white rounded-br-sm`
              ...
              {/* Input */}
              <div className={`${isLevel1User ? "user-level-1-floating-chat-input-wrap" : ""} ${isLevel2User ? "user-level-2-floating-chat-input-wrap" : ""} ${isLevel3User ? "user-level-3-floating-chat-input-wrap" : ""} px-3 py-3 border-t border-white/10 flex gap-2`}>
                <input
                  value={input}
                  ...
                  maxLength={1000}
                  className={`${isLevel1User ? "user-level-1-floating-chat-input" : ""} ${isLevel2User ? "user-level-2-floating-chat-input" : ""} ${isLevel3User ? "user-level-3-floating-chat-input" : ""} flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder-white/30 outline-none focus:border-purple-500 transition-colors`}
                />
                <button
                  onClick={handleSend}
                  disabled={!input.trim() || sending}
                  className={`${isLevel1User ? "user-level-1-floating-chat-send" : ""} ${isLevel2User ? "user-level-2-floating-chat-send" : ""} ${isLevel3User ? "user-level-3-floating-chat-send" : ""} p-2 rounded-xl transition-all disabled:opacity-40`}
      ...
      <button
        ...
        className={`${isLevel1User ? "user-level-1-floating-chat-button" : ""} ${isLevel2User ? "user-level-2-floating-chat-button" : ""} ${isLevel3User ? "user-level-3-floating-chat-button" : ""} relative w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-all hover:scale-110 active:scale-95`}
...
function FriendRow({ friend, online, unread, onClick, canInvite, onInvite }) {
  return (
    <div className="user-level-1-floating-chat-friend-row user-level-2-floating-chat-friend-row flex items-center hover:bg-white/5 transition-colors px-4">
