...
export default function PongGame({ onScoreUpdate , useLevelTheme = true }) {
 ...
 const isLevel1User = useLevelTheme && userLevel === 1;
  ...
  const draw = useCallback(() => {
    ...
    } else if (isLevel3User) {
      const skyGradient = ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
      skyGradient.addColorStop(0, "#031425");
      skyGradient.addColorStop(0.5, "#071f35");
      skyGradient.addColorStop(1, "#020b14");
    ...
    ctx.strokeStyle = isLevel1User ? "rgba(214, 171, 115, 0.16)" : isLevel3User ? "rgba(142, 221, 255, 0.18)" : "rgba(255,255,255,0.1)";
    ...
    ctx.shadowColor = isLevel1User ? "#c99662" : isLevel3User ? "#8fdcff" : "#a855f7";
    ctx.shadowBlur = 15;
    ctx.fillStyle = isLevel1User ? "#b7854f" : isLevel3User ? "#8fdcff" : "#a855f7";
    ctx.beginPath();
    ctx.roundRect(paddleMargin, game.paddle1.y, game.paddleWidth, game.paddleHeight, 6);
    ctx.fill();

    ctx.shadowColor = isLevel1User ? "#e0bf82" : isLevel3User ? "#55c9f4" : "#06b6d4";
    ctx.fillStyle = isLevel1User ? "#dcb979" : isLevel3User ? "#55c9f4" : "#06b6d4";
    ctx.beginPath();
    ctx.roundRect(canvas.width - paddleMargin - game.paddleWidth, game.paddle2.y, game.paddleWidth, game.paddleHeight, 6);
    ctx.fill();

    ctx.shadowColor = isLevel1User ? "#f8ddb0" : isLevel3User ? "#e7f8ff" : "#f43f5e";
    ctx.shadowBlur = 20;
    ctx.fillStyle = isLevel1User ? "#f2d3a1" : isLevel3User ? "#e7f8ff" : "#f43f5e";
    ctx.beginPath();
    ctx.arc(game.ball.x, game.ball.y, game.ballSize / 2, 0, Math.PI * 2);
    ctx.fill();
    ctx.shadowBlur = 0;
  }, [isLevel1User, isLevel3User]);
