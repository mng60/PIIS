...
function Avatar({ url, name, size = "md", isLevel2User = false, isLevel3User = false }) {
  ...
  return (
    <div className={`${sz} rounded-full flex items-center justify-center font-bold text-white ${isLevel2User ? "user-level-2-friends-avatar" : ""} ${isLevel3User ? "user-level-3-friends-avatar" : ""} ${!isLevel2User && !isLevel3User ? "bg-gradient-to-br from-purple-600 to-cyan-500" : ""}`}>
      {name?.[0]?.toUpperCase() || "?"}
    </div>
  );
}
    ...
    return (
      <div className={`flex items-center gap-3 p-3 rounded-xl bg-white/5 hover:bg-white/8 transition-colors ${isLevel2User ? "user-level-2-friends-row" : ""} ${isLevel3User ? "user-level-3-friends-row" : ""} ${isLevel4User ? "user-level-4-friends-row" : ""} ${isLevel5User ? "user-level-5-friends-row" : ""}`}>
        <button className="flex-shrink-0" onClick={() => navigate(`/profile/${encodeURIComponent(u.email)}`)}>
          <Avatar url={u.avatar_url} name={u.full_name} isLevel2User={isLevel2User} isLevel3User={isLevel3User} />
        </button>
